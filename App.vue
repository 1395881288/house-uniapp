<script>
	...
	export default {
		...
		onLaunch: function(option) {
			...
			// #ifdef H5
			// 静默登录的情况一：微信公众号
			console.log("===================微信公众号"+JSON.stringify(option))
			...
			if (!this.$store.getters.isLogin) {
				...
				Routine.getCode().then(code => {
					console.log("===================Routine.getCode() "+JSON.stringify(code))
					Routine.authUserInfo(code, spread)
						.then(res => {
							console.log("===================Routine.authUserInfo "+JSON.stringify(res))
						})
						.catch(err => {
						    console.log("===================Routine.authUserInfo "+JSON.stringify(err))
						});
				}).catch(res => {
					uni.hideLoading();
				});
...
<style lang="scss">
	...
	@import 'static/iconfontwoju/iconfont.css';
