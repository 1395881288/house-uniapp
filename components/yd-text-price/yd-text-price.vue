<template>
  <view>
    <text v-for="(item, index) in textArray" :key="index" :style="{ fontSize: (index === 1 ? integerSize : size) + 'px', color, textDecoration,fontWeight:'bold' }">
      {{ item }}
    </text>
    <text :style="{ fontSize:size + 'px', color, textDecoration,paddingLeft:'6rpx' }">元/月</text>
  </view>
</template>
