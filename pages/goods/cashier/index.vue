	...
	<view class="page" v-if="payPrice">
		<view class="pay-price">
			<view class="price">
				...
				<numberScroll :num='(payPrice / 100.0).toFixed(2)' color="#E93323" width='30' height='50'
					fontSize='50' />
			</view>
			<view class="count-down">
				支付剩余时间：
				<countDown :is-day="false" :tip-text="' '" :day-text="' '" :hour-text="' : '" :minute-text="' : '"
					:second-text="' '" :datatime="invalidTime / 1000" :is-col="true" :bgColor="bgColor" />
			</view>
		</view>
		<view class="payment" v-if="channelCode.length > 0">
			...
			<view class="item acea-row row-between-wrapper" v-for="(item,index) in channels" :key="index"
				...
				<view class="iconfont"
					:class="item.code === channelCode ? 'icon-xuanzhong11 font-num':'icon-weixuan'" />
			</view>
		</view>
		<view class="btn">
		  ...
		  if (options.scene == '1038' && options.referrerInfo.appId=='wxef277996acc166c3') {
		    ...
		    if (!extraData) {
		      ...
		      if (extraData.code == 'success') {
				...
				console.log("支付失败：" + extraData.errmsg);
				return that.$util.Tips({
					title: "支付失败",
					icon: 'error'
				});
		...
		methods: {
			...
			handleSubmitOrderResult(data) {
				...
				if (displayMode === 'app') {
					...
					if (this.channelCode === 'wx_lite_tl') {
						this.handleSubmitOrderResultForWxLiteTL(displayContent)
						return;
					}
				}
			},
			 ...
			 * 发起微信小程序支付
			 */
			handleSubmitOrderResultForWxLiteTL(displayContent) {
				const payConfig = JSON.parse(displayContent);
				const params=this.stringToJSON(payConfig.paySign);
				console.log("appId ",payConfig.appId)
				console.log(params)
				console.log("跳转通联支付："+"appId wxef277996acc166c3")
			...
			removeDisableChannel(appId) {
				...
				// #ifndef MP
				this.channels = this.channels.filter(item =>  item.code !== 'wx_lite' && item.code !== 'wx_lite_tl')
				// #endif
				// #ifdef MP
