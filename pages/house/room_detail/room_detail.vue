<template>
  <view class="container">
    ...
    <view class="descsub">
       ...
       <view class="itemgap"></view>
       <view class="item">
          <view class="itemtitle">{{detail.floor+"/"+detail.building.floorNum}}层</view>
...
<!--        <view class="btn-item" clickdel="doShare">
          <u-icon name="share" :size="24"></u-icon>
          <view class="btn-text">分享</view>
        </view> -->
		<view class="btn-item">
		    <button size="default" type="default"  class="wxshare" open-type="share">
				<u-icon name="share" :size="24"></u-icon>
				<view class="btn-text">分享</view>
			</button>   
		</view>
		...
		<view class="btn-item-main">
		  <u-button type="success" shape="circle" size="small" text="预定" @click="booking"></u-button>
		</view>
        <view class="btn-item-main">
          <u-button type="warning" shape="circle" size="small" text="在线聊"></u-button>
        </view>
        <view class="btn-item-main">
          <u-button type="error" color="#ea322b" shape="circle" size="small" text="打电话" @click="call"></u-button>
        </view>
      </view>
...
/* 	   this.$util.Tips({
	   ...
	   uni.share({
	   	...
	   	imageUrl: "https://api.woju.vip/logo.png",
	   	success: function (res) {
    ...
    call(){
	    ...
	    if (!myreg.test(this.detail.building.managerPhone)) {
		   uni.showToast({
		   	title: '暂无联系电话',
			icon:'none',
		   	duration: 2000
		...
		uni.makePhoneCall({
			phoneNumber: this.detail.building.managerPhone //仅为示例
		});
	},
	booking(){
		var params={buildingId:this.detail.buildingId,buildingName:this.detail.buildingName,roomId:this.detail.id,roomName:this.detail.roomName}
		params = encodeURIComponent(JSON.stringify(params));
		console.log(params)
		uni.navigateTo({
			url:'/pages/house/users/booking/add?params='+params
		})
	}
	//////////////////////////////////////////////
  }
