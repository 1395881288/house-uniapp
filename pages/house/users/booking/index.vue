<template>
	<view class="container">
		<u-sticky  customNavHeight="0" bgColor="#fff" v-if="0==1">
		    <u-tabs 
			:list="tabs"
			lineColor="#ffffff"
			lineHeight="0"
			...
			<block v-for="(data, index) in list" :key="index">
				...
				<view class="uni-list-cell">
					<view class="content">
						...
						<view class="date">预定时间：{{$u.timeFormat(data.bookDate, 'yyyy/mm/dd')}}</view>
						<view class="date">最晚签约日：{{$u.timeFormat(data.lastSigningDate, 'yyyy/mm/dd')}}</view>
						
						<view class="date">备注：{{data.remark}}</view>
					</view>
					<view class="price">
       ...
       <u-gap height="60"></u-gap>
		<view class="btnview">
			  <u-button class="btn" type="primary" shape="circle" text="添加预定" @click="navTo('/pages/house/users/booking/add')"></u-button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			getList() {
				...
				request.get('app-api/house/booking/page', this.queryParams)
				.then(res => {
					console.log(res)
