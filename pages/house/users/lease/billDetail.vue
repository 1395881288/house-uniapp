		...
		<view class="uni-list">
			<block v-for="(data, index) in billMaster.feeBills" :key="index">
				...
				<view class="uni-list-cell" v-if="[1201,1202,1203,1204].includes(data.feeType)">
					<view class="select" v-if="bill.status!=4">
					    <u-checkbox-group
					...
					<view class="price">
						<span style="margin-right: 10px;" :class="'status'+data.payStatus">¥ {{data.totalCost/100}}</span>
						<u-tag v-if="data.payStatus==0" text="待付款" plain size="mini" type="error"></u-tag>
						<u-tag v-else-if="data.payStatus==1" text="已付款" plain size="mini" type="success"></u-tag>
					</view>
				</view>
...
<!-- 		<u-empty v-if="loadStatus != 'loading' && billMaster.feeBills.length==0" mode="data" text=" " marginTop="150">
       ...
       <view v-if="loadStatus=='nomore' && bill.status!=4" style="display: flex;align-items: center;justify-content: center;">
	...
	export default {
		...
		methods: {
			...
			getFeeList() {
				...
				request.get('app-api/house/fee-type/list', queryParams)
				.then(res => {
					console.log(res)
					this.feeTypeList = res.data;
					this.getBillMaster(this.options.masterId)
			...
			getBillMaster(id){
				...
				.then(res => {
					...
					res.data.transactions.forEach(item => {
						...
						item.transInfos.forEach(transInfo => {
							//that.form.feeBillIds=that.form.feeBillIds.concat(transInfo.feeBillId);
							that.payedFeeBillIds=that.payedFeeBillIds.concat(transInfo.feeBillId);
							console.log("================= "+transInfo.feeBillId)
						})

					});
					that.payedFeeBillIds = [...new Set(that.payedFeeBillIds)];
					res.data.feeBills.forEach(fbitem => {
						fbitem.selected=[];
					    fbitem.disabled=that.payedFeeBillIds.includes(fbitem.id);
						if(!that.payedFeeBillIds.includes(fbitem.id))
						{
							fbitem.selected.push(fbitem.id);
							var fee=this.feeTypeList.find((fee) => fee.code==fbitem.feeType);
							var title="未定义";
							if(fee!=null)
							{
								title=fee.name
							...
							fbitem.title=title;
							tempids.push(fbitem.id);
							temptypes.push(fbitem.feeType)
							console.log("==============transactionFeeTypes not includes "+fbitem.feeType)
						}
						else
						{
 
							console.log("==============transactionFeeTypes includes "+fbitem.feeType)
						}
					})
					console.log(res.data)
					this.billMaster=res.data
					this.form.feeBillIds=tempids;
				    this.form.feeTypes=temptypes;
			...
			submit(){
				...
				json.feeTypes=[...new Set(feeTypes)];
				json.feeBillIds=[...new Set(feeBillIds)];
				...
				transactionApi.createOrder({
	              ...json
					...
					console.log(res)
 				  const returnUrl = encodeURIComponent('/pages/house/users/lease/orderPayStatus?order_id=' + res.data.id);
				  uni.navigateTo({
