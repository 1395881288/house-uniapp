		...
		<view class='payment-status'>
			...
			<view class='wrapper'>
				<view class='item acea-row row-between-wrapper'>
					<view>订单编号</view>
					<view class='itemCom'>{{order_pay_info.id}}</view>
				</view>
				<view class='item acea-row row-between-wrapper'>
					<view>下单时间</view>
					<view class='itemCom'>{{ order_pay_info.createTime ? formatDate(order_pay_info.createTime) : '-' }}
					</view>
				</view>
				...
				<view class='item acea-row row-between-wrapper'>
					<view>支付金额</view>
					<view class='itemCom'>{{ fen2yuan(order_pay_info.transactionAmount) }}</view>
				</view>
				<!-- 失败时加上这个 -->
				<view class='item acea-row row-between-wrapper' v-if="!order_pay_info.payStatus">
					<view>失败原因</view>
					<view class='itemCom' v-if="payResult === 'success'">获取支付结果失败，请稍后刷新</view>
					<!-- 一般情况下，是支付中心回调更新订单为已支付，存在延迟 -->
	...
	export default {
		...
		methods: {
			...
			getOrderPayInfo: function() {
				...
				transactionApi.get(this.orderId).then(res => {
					uni.hideLoading();
			...
			goOrderDetails: function() {
				uni.navigateTo({
					url: '/pages/house/users/lease/bill?leaseId=' + this.order_pay_info.leaseId
				})
			},
			goOrderDetailsold: function() {
				// #ifdef MP
				uni.showLoading({
					title: '正在加载',
				})
