<template>
    <view class="HMfilterDropdown" :class="{'setDropdownBottom':maskVisibility}" :style="{'top':menuTop+'px'}"
            ...
            <view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
                <block v-if="page.type=='hierarchy'&& page.submenu.length>0">
				    <view class="hierarchy">
						...
						<block v-for="(sub,index) in page.submenu" :key="'second_index0'+index">
							<scroll-view class="sub-menu-list not-first" :scroll-y="true"
								...
								<block v-for="(sub_second,second_index) in sub.submenu" :key="'second_index_1_'+second_index">
									<view class="sub-menu" :id="'second_id'+second_index"
										  :class="{'on':activeMenuArr[page_index][0]==index && activeMenuArr[page_index][1]==second_index}">
										<view class="menu-name"
						...
						<block v-for="(sub,index) in page.submenu" :key="'three_id_0_'+index">
							<scroll-view class="sub-menu-list not-first" :scroll-y="true"
										 v-if="activeMenuArr[page_index][0]==index&&sub.submenu.length>0"
										 :scroll-into-view="'three_id'+threeScrollInto">
								<block v-for="(sub_second,second_index) in sub.submenu" :key="'three_id_1_'+second_index">
									<block v-if="activeMenuArr[page_index][1]==second_index&&sub_second.submenu.length>0">
										<block v-for="(sub_three,three_index) in sub_second.submenu" :key="'three_id_3_'+second_index+'_'+three_index">
											<view class="sub-menu" :id="'three_id'+three_index"
												  :class="{'on':activeMenuArr[page_index][0]==index && activeMenuArr[page_index][1]==second_index&&activeMenuArr[page_index][2]==three_index}">
												<view class="menu-name"
													  @tap="selectHierarchyMenu(page_index,activeMenuArr[page_index][0],second_index,three_index)">
													<text>{{sub_three.name}}</text>
													<text class="iconfont selected"></text>
												</view>
...
<!--                        <view class="btn-box">
                ...
                <view class="btn-box">
                    <view class="reset" @tap="reset(page_index)">
						<view>重置</view>
					</view>
                    <view class="submit" @tap="submit(page_index)">查看房源</view>
                </view> 
            </view>
        </block>
	</view>
    ...
    export default {
        ...
        methods: {
            initMenu() {
				...
				this.tmpMenuActiveArr=JSON.parse(JSON.stringify(tmpMenuActiveArr));
                //加载菜单数据
				...
				console.log("===============this.activeMenuArr============="+JSON.stringify(this.activeMenuArr))
            },
            setMenuName() {
            ...
            selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				console.log("===========selectHierarchyMenu",page_index, level1_index, level2_index, level3_index)
                console.log(this.activeMenuArr)
                console.log(this.shadowActiveMenuArr)
				//读取记录

				if (level1_index != null && level2_index == null && level3_index == null && this.shadowActiveMenuArr[page_index][0] ==level1_index) {
                    console.log("111111111111111")
					var temparr=JSON.parse(JSON.stringify(this.shadowActiveMenuArr[page_index]));
					temparr[0]=level1_index;
					temparr[1]=null;
					temparr[2]=null;
					this.activeMenuArr.splice(page_index, 1, temparr);
                }
				else if(level1_index != null && level2_index != null && level3_index == null && this.shadowActiveMenuArr[page_index][1] ==level2_index) {
                    console.log("222222222222222")
					var temparr=JSON.parse(JSON.stringify(this.shadowActiveMenuArr[page_index]));
					temparr[0]=level1_index;
					temparr[1]=level2_index;
					temparr[2]=null;
					this.activeMenuArr.splice(page_index, 1, temparr);
				
                }
				else if(level1_index != null && level2_index != null && level3_index != null && this.shadowActiveMenuArr[page_index][2] ==level3_index) {
                    console.log("333333333333333")
					var temparr=JSON.parse(JSON.stringify(this.shadowActiveMenuArr[page_index]));
					temparr[0]=level1_index;
					temparr[1]=level2_index;
					temparr[2]=level3_index;
					this.activeMenuArr.splice(page_index, 1, temparr);
				...
				if (level1_index != null && this.subData[page_index].submenu[level1_index].submenu.length == 0 && level2_index == null && level3_index == null)
				{
				...
				if (level1_index != null && level2_index != null && this.subData[page_index].submenu[level1_index].submenu[level2_index].length == 0 && level3_index == null)
				{
                ...
                //写入结果
                if (level1_index != null && level2_index != null && level3_index != null)
				{
				...
				console.log("this.activeMenuArr "+JSON.stringify(this.activeMenuArr))
				console.log("this.shadowActiveMenuArr "+JSON.stringify(this.shadowActiveMenuArr))
            },
            //写入结果，筛选
			...
			reset(index){
				if(index==2)
				{
					this.resetFilterData(index)
				}
				else
				{
					
					 this.resetPage(this.showPage);
				}
			},
			submit(index){
				if(index==2)
				{
					this.setFilterData(index)
				}
				else
				{
                ...
                if (this.menu[index].type == 'filter') {
                    ...
                    this.$nextTick(() => {
                        setTimeout(() => {
							...
							this.threeScrollInto = parseInt(this.activeMenuArr[index][2]);
                        }, 0);
                    })
			...
			//重置结果和ui，筛选
			resetPage(page_index) {
				console.log("resetPage "+this.showPage+" "+page_index)
          
				this.activeMenuArr[page_index] = JSON.parse(JSON.stringify(this.tmpMenuActiveArr[page_index]));
				this.shadowActiveMenuArr[page_index] = JSON.parse(JSON.stringify(this.tmpMenuActiveArr[page_index]));
 				this.subData[page_index]=JSON.parse(JSON.stringify(this.defaultFilterData[page_index]))
				if(page_index==0)
				{
					this.activeMenuArr[page_index] = [0,0,0];
					this.shadowActiveMenuArr[page_index] = [1,1,null];
				}
				console.log()
				this.$forceUpdate(this.subData[page_index]);
			},
            discard() {
